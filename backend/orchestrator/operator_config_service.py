        ...
        *,
        ...
        if path_policy == "bundle_internal_path":
            if key in {
                ...
                if raw_value.startswith("/opt/llm-tools-platform/external/"):
                    return {
                        "status": "info",
                ...
                [
                    {
                        ...
                        "fields": [
                            ...
                            self._field(
                                ...
                                "Chainlit auth secret",
                                "backend/.env",
                                suggested=env_value(backend_env, "CHAINLIT_AUTH_SECRET", default="llm-tools-platform-secret-key-change-me"),
                            ...
                            self._field(
                                ...
                                "Open WebUI secret key",
                                "backend/.env",
                                suggested=env_value(backend_env, "WEBUI_SECRET_KEY", default="llm-tools-platform-openwebui-dev-secret-change-me"),
                                recommended_reason="Задавай явно, чтобы recovery/admin bootstrap не зависели от временного секрета.",
                                recommended_reason_en="Set it explicitly so recovery and admin bootstrap do not rely on a temporary secret.",
                                secret=True,
                            ),
                            self._field(
                                backend_env,
                                ...
                                "Open WebUI admin email",
                                "backend/.env",
                                suggested=env_value(backend_env, "WEBUI_ADMIN_EMAIL", default="admin@example.com"),
                                recommended_reason="Для bootstrap admin лучше держать явный email, а не искать старого пользователя в volume.",
                                recommended_reason_en="Use an explicit bootstrap admin email instead of hunting for an old user in the volume.",
                            ),
                            self._field(
                                backend_env,
                                ...
                                "Open WebUI admin password",
                                "backend/.env",
                                suggested=env_value(backend_env, "WEBUI_ADMIN_PASSWORD", default="change-me-now"),
                                recommended_reason="Меняй пароль после bootstrap и не оставляй dev-default на общей машине.",
                                recommended_reason_en="Rotate the password after bootstrap and do not keep the dev default on a shared machine.",
                                secret=True,
                            ),
                            self._field(
                                backend_env,
                                ...
                                "Open WebUI admin name",
                                "backend/.env",
                                suggested=env_value(backend_env, "WEBUI_ADMIN_NAME", default="llm-tools-platform Admin"),
                                recommended_reason="Явное имя admin упрощает recovery и handoff локального eval contour.",
                                recommended_reason_en="An explicit admin name makes local eval contour recovery and handoff easier.",
                            ),
                            self._field(
                                backend_env,
                                "ENABLE_SIGNUP",
                                "Open WebUI sign-up enabled",
                                "Open WebUI sign-up enabled",
                                "backend/.env",
                                suggested=env_value(backend_env, "ENABLE_SIGNUP", default="true"),
                                recommended_reason="Оставляй signup включённым только если это осознанный локальный/shared eval policy.",
                                recommended_reason_en="Keep sign-up enabled only when this is an intentional local or shared eval policy.",
                                control="toggle",
                            ...
                            self._field(
                                backend_env,
                                "DEFAULT_USER_ROLE",
                                "Open WebUI default user role",
                                "Open WebUI default user role",
                                "backend/.env",
                                suggested=env_value(backend_env, "DEFAULT_USER_ROLE", default="pending"),
                                recommended_reason="Для безопасного bootstrap держи новые учётки в pending, а не выдавай admin по умолчанию.",
                                recommended_reason_en="For safer bootstrap, keep new accounts in pending instead of granting admin by default.",
                                control="select",
                                options=self._select_options(["pending", "user", "admin"]),
                            ),
                            self._field(
                                backend_env,
                                "OPERATOR_UI_LOCALHOST_ONLY",
                                "Operator UI localhost-only",
                                "Operator UI localhost-only",
                                "backend/.env",
                                suggested=env_value(backend_env, "OPERATOR_UI_LOCALHOST_ONLY", default="true"),
                                recommended_reason="Пока нет полноценного auth/ACL слоя, operator surface должен быть закрыт для non-localhost клиентов.",
                                recommended_reason_en="Until a full auth/ACL layer exists, the operator surface should stay closed to non-localhost clients.",
                                control="toggle",
                                options=self._boolean_options(),
                            ),
                            self._field(
                ...
                [
                    ...
                    self._preset(
                        ...
                        "model_source_mode",
                        {
                            ...
                            "MODEL_PATH_VLM": "/opt/llm-tools-platform/external/vlm/model.gguf",
                            "MMPROJ_PATH": "/opt/llm-tools-platform/external/vlm/mmproj.gguf",
                            "MODEL_PATH_EMBEDDING_INTENT": "/opt/llm-tools-platform/external/embedders/intent",
                            "MODEL_PATH_EMBEDDING_RETRIEVAL": "/opt/llm-tools-platform/external/embedders/retrieval",
                        },
                ...
                [
                    {
                        ...
                        "fields": [
                            ...
                            self._field(bundle_env, "BUNDLE_MODEL_ROOT", "Bundle model root", "Bundle model root", "deploy/offline_bundle/env.bundle", suggested=env_value(bundle_env, "BUNDLE_MODEL_ROOT", default="/opt/llm-tools-platform/models"), recommended_reason="Путь должен совпадать с runtime layout bundle.", recommended_reason_en="This path must stay aligned with the bundle runtime layout.", path_policy="bundle_internal_path", path_example="/opt/llm-tools-platform/models", visible_when={"MODEL_SOURCE_MODE": "bundle_layout"}),
                            self._field(bundle_env, "BUNDLE_UPLOADS_ROOT", "Uploads root", "Uploads root", "deploy/offline_bundle/env.bundle", suggested=env_value(bundle_env, "BUNDLE_UPLOADS_ROOT", default="/opt/llm-tools-platform/uploads"), recommended_reason="Путь должен совпадать с runtime layout bundle.", recommended_reason_en="This path must stay aligned with the bundle runtime layout.", path_policy="bundle_internal_path", path_example="/opt/llm-tools-platform/uploads"),
                            self._field(bundle_env, "BUNDLE_REPORTS_ROOT", "Reports root", "Reports root", "deploy/offline_bundle/env.bundle", suggested=env_value(bundle_env, "BUNDLE_REPORTS_ROOT", default="/opt/llm-tools-platform/reports"), recommended_reason="Путь должен совпадать с runtime layout bundle.", recommended_reason_en="This path must stay aligned with the bundle runtime layout.", path_policy="bundle_internal_path", path_example="/opt/llm-tools-platform/reports"),
                        ],
                    },
                    {
