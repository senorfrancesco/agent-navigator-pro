...
const LEGACY_DEEP_JOB_ACTION_TESTS_ENABLED =
  process.env.OPENWEBUI_ENABLE_LEGACY_DEEP_JOB_ACTION_TESTS?.trim() === '1';
...
const LEGACY_ACTION_FUNCTION_SYNC_RULES: Record<string, string[]> = {
...
test.beforeAll(async ({ request }) => {
  ...
  if (LEGACY_DEEP_JOB_ACTION_TESTS_ENABLED) {
    await assertInstalledActionFunctionsSynced(request, authToken);
  } else {
...
test('openwebui ui-controls refresh deep-job materializes terminal result', async ({ page, request }) => {
  test.skip(
    !LEGACY_DEEP_JOB_ACTION_TESTS_ENABLED,
    'Legacy deep-job Action Function сценарии выключены по умолчанию.'
  );
