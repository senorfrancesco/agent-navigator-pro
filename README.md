...
## Конфиг классификации для `document_analysis`

Ключевые слова для rule-based классификации типа документа задаются в `backend/orchestrator/data/parsers_config.yaml` в секции `document_type_keywords`:

```yaml
document_type_keywords:
  tz: ["техническое задание", "предмет закупки", "требования к поставляемому"]
  smeta: ["сметный расчет", "сметный расчёт", "стоимость работ"]
  kp: ["коммерческое предложение", "ценовое предложение"]
  legal: ["договор", "контракт", "предмет договора"]
```

Правила:
- ключи (`tz`, `smeta`, `kp`, `legal`) — типы документов, возвращаемые workflow;
- значения — список строковых keyword-паттернов (поиск через `in`, регистр игнорируется);
- если секция отсутствует, пуста или конфиг повреждён, workflow применяет встроенный fallback-набор keywords и пишет warning в лог.
