...
- [x] **T6.4 P5 — Legacy Open WebUI: целостность запуска `deep-job` для native tool call**
  ...
  Progress: добавлен локальный eval runner `tests/harness/openwebui/openwebui_deep_job_eval.py` и внешний `Playwright`-контур `tests/e2e/openwebui/deep-job.spec.ts`; после вывода legacy-контура runner и acceptance закреплены как native-only: runtime preflight, bootstrap, нативная панель long-running выполнения, backend poll через `tool_jobs`, terminal result и артефакты в `output/openwebui-deep-job-eval/*`.
  ...
  Progress: исторический live-run `output/openwebui-deep-job-eval/20260412T203316Z` подтвердил backend-contour для `requirements-p1`; дальнейшие проверки этого backlog item теперь нужно трактовать только через native panel-path, без `equipment_deep_action` и `tool_job_refresh_action`.
...
- [x] **T6.4 P6 — `equipment_deep_action` теряет файловый контекст и уводит `deep-job` в общий чатовый путь**
  ...
  Комментарий: выполнено через проброс `document_refs` / `session_docs` / `attachments_meta` из исторического `Open WebUI action` в `tool-server` и нормализацию путей `/app/backend/data/uploads/*` -> `backend/open_webui_uploads/*`; живой повторный запуск на паре `Requirements.pdf` + `Quotation_12.pdf` подтвердил настоящий equipment-конвейер с извлечением и сопоставлением обоих PDF. Сам `Action Function`-контур позже снят с поддержки в пользу нативной панели long-running выполнения.
