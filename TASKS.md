...
### UX/Support Notes (2026-03-04)

- [x] **U3.1 — Chainlit resume UX: единый системный статус восстановления**
  `backend/orchestrator/chainlit_app.py`: добавлен helper `_build_resume_status_message()` и отправка сообщения после `@cl.on_chat_resume`.
  Сообщение стандартизировано: восстановленные сообщения, найденные документы, готовность RAG-индекса.

- [x] **U3.2 — Локализация истории чатов под единые формулировки**
  `backend/orchestrator/.chainlit/translations/ru-RU.json`: `Список чатов`, `Найти чат...`, `Чат без названия`.

- [x] **U3.3 — Документация для команды/поддержки по переключению чатов**
  `backend/orchestrator/chainlit_ru-RU.md`: зафиксирован целевой UX-сценарий и порядок восстановления контекста при выборе треда.
