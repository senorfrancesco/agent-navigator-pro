...
class GlobalConfig:
    ...
    def auto_configure(cls):
        """Автоматическая настройка параметров под железо"""
        if cls.device_mode == "auto":
             cls.device_mode = "gpu" if torch.cuda.is_available() else "cpu"
        
        print(f"⚙️  Авто-конфигурация: Режим={cls.device_mode}")

        # Логика выбора OCR движка
        if cls.ocr_engine == "auto" or cls.ocr_strategy == "auto":
            vram_gb = 0
            if cls.device_mode == "gpu":
                try:
                    vram_gb = torch.cuda.get_device_properties(0).total_memory / (1024**3)
                except: pass
            
            # Если VRAM мало (<10GB), тяжелый Qwen-VL может не влезть вместе с LLM
            # Поэтому по дефолту берем Paddle, если явно не попросили иное
            if vram_gb < 10:
                print(f"   ⚠️ VRAM {vram_gb:.1f}GB < 10GB. Принудительно используем PaddleOCR для стабильности.")
                cls.ocr_engine = "paddle" 
            else:
                print(f"   ✅ VRAM {vram_gb:.1f}GB > 10GB. Доступен режим Qwen-VL.")
                # Оставляем paddle по умолчанию для скорости, qwen-vl включится только для сложных страниц
                if cls.ocr_engine == "auto":
                    cls.ocr_engine = "paddle" 
...
def build_graph(task_type: str):
    ...
    # Визуализация графа
    try:
        print("📊 Генерация схемы графа (graph.png)...")
        graph_png = app.get_graph().draw_mermaid_png()
        with open("graph.png", "wb") as f:
            f.write(graph_png)
        print("   ✅ Схема сохранена в graph.png")
    except Exception as e:
        print(f"   ⚠️ Не удалось сохранить схему графа (нужен graphviz/dot): {e}")
...
if __name__ == "__main__":
    ...
    GlobalConfig.ocr_engine = "auto" if args.ocr_engine == "paddle" and args.ocr_strategy == "auto" else args.ocr_engine # Хак для автовыбора
    GlobalConfig.device_mode = args.device
    
    GlobalConfig.auto_configure()
