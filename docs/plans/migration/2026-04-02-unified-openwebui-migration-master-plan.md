...
- [ ] Это размывает границу между `model behavior` и `backend business logic` и делает tool UX трудноотлаживаемым.
- [ ] Для корректного native tool-calling нужен отдельный raw `OpenAI-compatible` model provider surface.
...
- [ ] Нельзя считать историю диалога главным источником фактов по документу вместо retrieval-слоя.
- [ ] Нельзя без границ передавать весь накопленный чат в prompt на каждом ходу.

### Что нужно хранить отдельно

- [ ] `chat/session state`
  - [ ] история сообщений (`history`)
  - [ ] thread summary / сжатое состояние диалога
...
- [ ] `run/tool state`
  - [ ] orchestration checkpoints / `state_ref`
  - [ ] `pending_action`
  - [ ] `tool_jobs`
  - [ ] long-running артефакты

### Практическая модель

- [ ] История чата хранится в слое чата интерфейса и backend.
- [ ] История чата используется как дискурсивная память:
  - [ ] последние raw turns;
  - [ ] summary по старой части диалога;
  - [ ] ссылки на уже выбранные документы и сущности.
- [ ] Активные вложения хранятся как привязки (`bindings`).
- [ ] Ответ по документу строится не из одной истории диалога, а из:
  - [ ] thread summary;
  - [ ] нескольких последних raw turns;
  - [ ] retrieval-результатов;
...
- [ ] `session_rag`
  ...
  - [ ] на переходном этапе может жить как путь наложения (`overlay path`)
  - [ ] целевой источник истины для retrieval: backend-owned `Qdrant` scope с `source_scope=session`
- [ ] `knowledge_base_rag`
...
- [ ] Да, и это целевая архитектура.
- [ ] Лучшая практика:
  - [ ] в V1 допустим временный fallback: persisted KB в `Qdrant` и session overlay через `session_docs`, если session index ещё не готов;
  - [ ] объединение остаётся на слое правил (`policy layer`);
  - [ ] в V2 session retrieval работает уже только от backend-owned persisted session scopes.
- [ ] В V2 session документы живут в `Qdrant` с короткоживущими областями действия (`short-lived scopes`):
  - [ ] `thread_id`
  - [ ] `ttl`
  - [ ] `source_scope=session`
- [ ] После стабилизации session indexing `session_docs` больше не должен использоваться как retrieval fallback; он остаётся только транспортным входом upload/handoff.
...
- [ ] Нужно заранее решить:
  - [ ] TTL
  - [ ] re-upload semantics
  - [ ] дедупликацию и обновление версий
  - [ ] cleanup по завершившимся или брошенным веткам
- [ ] Но это ограничение относится только к retrieval-слою.
- [ ] `Qdrant` не должен становиться универсальной базой для:
  - [ ] истории чата;
  - [ ] orchestration state;
  - [ ] `tool_jobs`;
  - [ ] UI pending-state.
