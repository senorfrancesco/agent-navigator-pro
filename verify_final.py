...
async def verify():
    ...
    print("--- Running Final Verification with LLM Polisher ---")
        ...
        for node_name, output in event.items():
            if node_name == "extract":
                ...
                if items:
                    ...
                    print(f"Polished Specs:\n{server.get('specs', '')}")
                    
                    specs = server.get('specs', '').lower()
                    print("\nVerification (Keywords in Specs):")
                    for kw in ["tpm", "блоки питания", "pcie", "rack", "юнит"]:
                        status = "✅ FOUND" if kw in specs else "❌ MISSING"
                        print(f"  - {kw}: {status}")
            
            if "errors" in output and output["errors"]:
                print(f"Errors in {node_name}: {output['errors']}")
